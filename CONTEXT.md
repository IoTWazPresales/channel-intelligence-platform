...
| Date | Summary |
|------|---------|
| 2026-06-27 | **Memory palace refresh; `b2b81ea` pushed to `origin/feat/dsi-async-topology`:** CURRENT.md + MEMORY_PALACE.md synced (HEAD, dev preflights, ops restart notes). Commit bundles BACKLOG-050 fix (single-writer `staged_metadata`), DSI `ImportJobLoadedSuccessCallout`, dev-worker orphan-kill preflight, idempotent loaded re-apply. Proven E2E job #199; job #96 facts intact at `loaded`. Large-volume (178k) apply re-soak still open. Dev: API/worker node wrappers can exit on Windows — restart services; preflight killed orphan celery 23376+5032 on worker restart. |
