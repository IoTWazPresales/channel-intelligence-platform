            ...
            <Stack direction="row" spacing={0.75} alignItems="center">
              ...
              {l.count !== undefined ? (
                <Typography
                  component="span"
                  variant="caption"
                  sx={{ px: 0.75, borderRadius: 1, bgcolor: 'action.selected', fontVariantNumeric: 'tabular-nums' }}
                >
                  {l.count}
                </Typography>
              ) : null}
...
export function StatusChip({
  ...
  const color =
    tone === 'danger' ? 'error' : tone === 'warning' ? 'warning' : tone === 'success' ? 'success' : tone === 'info' ? 'primary' : 'default';
  return (
    <Chip
      size="small"
      label={label}
