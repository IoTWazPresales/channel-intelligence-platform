...
import type { StewardBulkTestIds, StewardCatalogOpt } from './stewardEngine.types';
import type { useStewardBulkSteward } from './useStewardBulkSteward';

type BulkSteward = ReturnType<typeof useStewardBulkSteward>;

export function StewardBulkActionInlineForm({
  bulk,
  regions,
  channels,
  onCancel,
  testIds,
  renderMapCustomerFields,
}: {
  bulk: BulkSteward;
  regions: StewardCatalogOpt[];
  channels: StewardCatalogOpt[];
  onCancel: () => void;
  testIds: StewardBulkTestIds;
  renderMapCustomerFields: (args: {
    searchQuery: string;
  ...
  return (
    ...
    >
      ...
      <FormControl size="small" fullWidth>
        <InputLabel id={testIds.actionSelectLabelId}>Bulk action</InputLabel>
        <Select
          labelId={testIds.actionSelectLabelId}
          label="Bulk action"
          value={bulkAction}
      ...
      {bulkAction === 'map_customer'
        ? renderMapCustomerFields({
            ...
            searchTestId: testIds.customerSearch,
            selectTestId: testIds.customerSelect,
          })
        : null}
      ...
      {bulkAction === 'set_plan_fallback_channel' ? (
        <Stack spacing={1}>
          ...
          <FormControl size="small" fullWidth>
            <InputLabel id={testIds.planChannelLabelId}>Fallback channel</InputLabel>
            <Select
              labelId={testIds.planChannelLabelId}
      ...
      {bulkAction === 'create_provisional_customer' ? (
        <Stack spacing={1}>
          <Alert severity="info" variant="outlined" data-testid={testIds.provCustomerHint}>
            One new <strong>unverified</strong> customer account per selected row; display names follow dealer/source
            evidence.
          </Alert>
          <FormControl size="small" fullWidth>
            <InputLabel id={testIds.regionLabelId}>Fallback region (optional)</InputLabel>
            <Select
              labelId={testIds.regionLabelId}
          ...
          <FormControl size="small" fullWidth>
            <InputLabel id={testIds.channelLabelId}>Fallback channel (optional)</InputLabel>
            <Select
              labelId={testIds.channelLabelId}
          ...
          <FormControl size="small" fullWidth>
            <InputLabel id={testIds.tierLabelId}>Partner tier</InputLabel>
            <Select
              labelId={testIds.tierLabelId}
              label="Partner tier"
      ...
      {bulkAction === 'create_provisional_distributor' ? (
        <Stack spacing={1}>
          <Alert severity="info" variant="outlined" data-testid={testIds.provDistHint}>
          ...
          <label>
            <input
              ...
              data-testid={testIds.distSuspicious}
            />{' '}
