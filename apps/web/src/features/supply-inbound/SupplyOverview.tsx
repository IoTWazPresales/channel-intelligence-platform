...
export function SupplyOverview() {
  ...
  return (
    <Box data-testid="domain-supply">
      {awaiting ? (
        <Typography color="text.secondary" sx={{ px: 1, py: 1 }} data-testid="supply-overview-loading">
          Loading Supply & Inbound…
        </Typography>
      ) : unavailable ? (
        <Typography color="text.secondary" sx={{ px: 1, py: 1 }} data-testid="supply-overview-empty">
          Supply & Inbound headlines are not available yet — inbound shipment facts are required.
        </Typography>
      ) : (
