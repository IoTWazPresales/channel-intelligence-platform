...
import {
  ...
  parseBulkPromoteCsv,
} from './DistributorBulkPromoteDialog';
...
function renderDialog() {
  ...
  return renderWithProviders(
    <QueryClientProvider client={qc}>
      <DistributorBulkPromoteDialog open onClose={vi.fn()} />
    </QueryClientProvider>
  );
}
...
describe('parseBulkPromoteCsv', () => {
  it('parses header and blank new_code', () => {
    ...
    expect(rows).toEqual([
      { tmp_code: 'TMP-DIST-A', new_code: 'ACME-1', note: undefined },
      { tmp_code: 'TMP-DIST-B', new_code: '', note: undefined },
    ]);
  });
});

describe('DistributorBulkPromoteDialog', () => {
  beforeEach(() => {
    apiPost.mockReset();
  });
  ...
  it('preview shows mixed ready/blocked and confirm posts dry_run false', async () => {
    ...
    fireEvent.change(screen.getByTestId('bulk-promote-paste'), {
      target: { value: 'TMP-DIST-A,ACME-1\nTMP-DIST-B,TAKEN' },
    });
    ...
    expect(apiPost).toHaveBeenLastCalledWith('/api/v1/distributors/promote/batch', {
      rows: [
        { tmp_code: 'TMP-DIST-A', new_code: 'ACME-1', note: undefined },
        { tmp_code: 'TMP-DIST-B', new_code: 'TAKEN', note: undefined },
      ],
      dry_run: false,
      mode: 'map',
    });
  ...
  it('mint mode previews and chunks 1200 into 3 requests', async () => {
    const codes = Array.from({ length: 1200 }, (_, i) => `TMP-DIST-${i}`);
    apiPost.mockImplementation(async (_url: string, body: { rows: { tmp_code: string }[] }) => ({
      ...
      rows: body.rows.map((r, i) => ({
        tmp_code: r.tmp_code,
        new_code: `DIST-${String(i + 1).padStart(6, '0')}`,
        distributor_id: i + 1,
...
describe('chunkRows', () => {
  it('splits 1200 into 3 chunks of 500/500/200', async () => {
    const { chunkRows } = await import('./DistributorBulkPromoteDialog');
