...
export function DistributorDispositionDialog({ open, onClose, distributorIds }: Props) {
  ...
  const previewMut = useMutation({
    mutationFn: async () => {
      if (!distributorIds.length) throw new Error('Select at least one distributor');
      if (distributorIds.length > 500) throw new Error('Max 500 per batch');
      return apiPost<BatchResponse>('/api/v1/distributors/disposition/batch', {
  ...
  const confirmMut = useMutation({
    mutationFn: async () =>
      apiPost<BatchResponse>('/api/v1/distributors/disposition/batch', {
        distributor_ids: distributorIds,
        disposition,
        note: note.trim() || undefined,
        dry_run: false,
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth data-testid="distributor-disposition-dialog">
      <DialogTitle>Park / exclude provisional distributors</DialogTitle>
      <DialogContent dividers>
        <Stack spacing={2} sx={{ mt: 0.5 }}>
          <Typography variant="body2" color="text.secondary">
            Marks TMP rows that should not receive a permanent code. Does not change distributor
            status (provisional reuse stays intact). {distributorIds.length} selected.
          </Typography>
          {step === 'input' ? (
            <>
              <FormControl>
          ...
          {(step === 'preview' || step === 'result') && displayRows.length ? (
            <Stack spacing={1} data-testid={step === 'result' ? 'disposition-result' : 'disposition-preview'}>
              ...
              <Table size="small">
                ...
                <TableBody>
                  {displayRows.map((r, i) => (
                    <TableRow key={`${r.distributor_id}-${i}`}>
                      <TableCell>{r.distributor_id ?? 'â€”'}</TableCell>
                      <TableCell>{r.tmp_code || 'â€”'}</TableCell>
                      <TableCell>{r.status}</TableCell>
                      <TableCell>{(r.reasons || []).join(', ') || 'â€”'}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      <DialogActions>
        ...
        {step === 'input' ? (
          <Button
            variant="contained"
            disabled={!distributorIds.length || previewMut.isPending}
        ...
        {step === 'preview' ? (
          <>
            ...
            >
              {confirmMut.isPending ? 'Applyingâ€¦' : `Apply ${readyN} ready`}
            </Button>
          </>
        ) : null}
      </DialogActions>
