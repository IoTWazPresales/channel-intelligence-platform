...
export type CommercialSummary = {
  ...
  week_start: string;
  week_end: string;
  filter_scope?: { active: boolean; cohort_line_count: number };
  total_lines: number;
  pipeline_in_transit: {
    line_count: number;
  ...
  landed_this_week?: { total: number };
  delivered_this_week?: { total: number };
  overdue: {
    count: number;
...
export function ShippingCommercialSummary({
  ...
  const pipelineSubtitle =
    sortedCur.length > 1
      ? `${sortedCur.length} currencies · ${pipe.line_count.toLocaleString()} scheduled lines with amount`
      : `${pipe.line_count.toLocaleString()} scheduled lines with amount`;
  ...
  const overduePctAll = Math.round((commercial.overdue.pct_of_total_lines ?? 0) * 1000) / 10;
  const overduePctPipe = Math.round((commercial.overdue.pct_of_scheduled_pipeline ?? 0) * 1000) / 10;
    ...
    <>
      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>
        {filtersActive ? (
          ...
          <>
            KPI cards reflect <strong>all</strong> inbound fact lines. Narrow filters on the grid (or click a card) to
            scope KPIs to that cohort.
          </>
        )}{' '}
      ...
      <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap sx={{ mb: 2 }}>
        <Box sx={cardWrapSx}>
          <KpiShell
            title="Pipeline value (in transit)"
        ...
        <Box sx={cardWrapSx}>
          <KpiShell
            ...
            subtitle={`${commercial.arriving_this_week.total.toLocaleString()} lines · ISO week ${commercial.week_start} → ${commercial.week_end}`}
        ...
        <Box sx={cardWrapSx}>
          <KpiShell
            ...
            subtitle={`${deliveredWeekTotal.toLocaleString()} lines · POD in current ISO week`}
            accent={successMain}
        ...
        <Box sx={cardWrapSx}>
          <KpiShell
            ...
            subtitle={`${overduePctPipe}% of scheduled pipeline · ${overduePctAll}% of all fact lines`}
            accent={errorMain}
