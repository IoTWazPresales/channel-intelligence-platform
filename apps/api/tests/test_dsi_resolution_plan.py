...
def test_plan_product_running_change_suggests_ignore_not_token_alias() -> None:
    sess = MagicMock()
    job = MagicMock()
    job.source.id = 9
    prod_idx = MagicMock()
    cand = _cand(
        ...
        context={
            ...
            "receipt_disambiguation": {
                ...
                "receipt_product_ids": [10, 20],
            },
            "fifo_candidate": True,
        },
    )
    with patch("app.services.imports.dsi_resolution_plan.dsi_first_sample", return_value="P1503CTA-I38512G1W"):
        out = plan_dsi_candidate_sync(sess, cand, job, prod_idx, default_region_id=None, default_channel_id=None)
    assert out["suggested_action"] == "ignore"
    assert out["token_level_resolve_product_blocked"] is True
    assert out["suggested_ignore_reason_code"] == "ignore_sku_indeterminate"
    assert "running_change_token_alias_blocked" in (out.get("resolution_blockers") or [])
