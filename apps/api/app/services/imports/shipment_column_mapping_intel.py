"""Shipment import: column-level confidence, reasons, and auto-map (aligned with PM explainability style)."""
...
def _reason_label(code: str) -> str:
    labels: dict[str, str] = {
        ...
        "header_heuristic": "Matched a built-in header pattern for shipment extracts.",
...
def suggest_shipment_column_mapping(
    headers: list[str],
    ...
    *,
    ...
    for canonical, meta in template.items():
        if canonical not in SHIPMENT_CANONICAL_TARGETS or not isinstance(meta, dict):
            continue
    ...
    baseline = build_initial_shipment_field_mapping(headers, source)
    ...
    for col in headers:
        ...
        if mem_row and mem_row.get("target"):
            ...
            if tgt in SHIPMENT_CANONICAL_TARGETS:
        ...
        if heur and heur in SHIPMENT_CANONICAL_TARGETS:
...
def apply_high_confidence_shipment_automap(
    headers: list[str],
    source: SourceDefinition | None,
    ...
    *,
    ...
    hints = suggest_shipment_column_mapping(headers, source, column_samples=column_samples, current_field_mapping=base_mapping)
