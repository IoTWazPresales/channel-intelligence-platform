"""Enqueue CST resolution-plan compute/apply (Unit E2, mirror CPOR D-013).

Broker first, dev in-process thread on enqueue failure (when
``CIP_DEV_CELERY_DISPATCH=in_process_thread``), else inline sync fallback.
"""
...
from app.services.imports.cst_resolution_plan_apply_sync import (
    run_cst_resolution_plan_apply_sync as _apply_sync,
)
from app.services.imports.cst_resolution_plan_compute_sync import (
    run_cst_resolution_plan_compute_sync as _compute_sync,
)
...
TASK_CST_RESOLUTION_PLAN_COMPUTE = "imports.cst_resolution_plan_compute"
TASK_CST_RESOLUTION_PLAN_APPLY = "imports.cst_resolution_plan_apply"

_dev_cst_resolution_plan_task_results: dict[str, dict[str, Any]] = {}


def dev_cst_resolution_plan_task_results() -> dict[str, dict[str, Any]]:
    return _dev_cst_resolution_plan_task_results


def run_cst_resolution_plan_compute_sync(
    job_id: int,
...
def enqueue_cst_resolution_plan_task(
    *,
    task_name: str,
    job_id: int,
    payload: dict[str, Any],
    ...
    try:
        ...
        logger.exception("cst_resolution_plan: Celery enqueue failed job_id=%s task=%s", job_id, task_name)
        if settings.cip_dev_celery_dispatch == "in_process_thread":
            ...
            def _in_process() -> None:
                try:
                    out = run_sync()
                    _dev_cst_resolution_plan_task_results[task_id] = {"state": "SUCCESS", "result": out}
                except Exception as exc:  # noqa: BLE001
                    logger.exception(
                        "cst_resolution_plan in-process thread failed job_id=%s task_id=%s", job_id, task_id
                    )
                    _dev_cst_resolution_plan_task_results[task_id] = {
                        "state": "FAILURE",
                        "error": str(exc)[:800],
                    }
                    raise
            ...
            DEV_CELERY_LOGGER.warning(
                "ENQUEUE: cst_resolution_plan %s job_id=%s — in-process thread (DEV ONLY).",
                task_name,
                job_id,
            )
            spawn_in_process_thread_with_ledger(
