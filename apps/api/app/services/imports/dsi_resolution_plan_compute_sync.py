"""Background DSI resolution-plan compute (read-only plan generation off the HTTP path)."""

from __future__ import annotations

import logging
from typing import Any, Callable

from app.db.session_sync import SessionLocal
from app.services.imports.dsi_resolution_plan import build_dsi_resolution_plan_sync

logger = logging.getLogger(__name__)


def run_dsi_resolution_plan_compute_sync(
    job_id: int,
    payload: dict[str, Any],
    *,
    on_progress: Callable[[int, int], None] | None = None,
) -> dict[str, Any]:
    """Build a transient steward resolution plan in a sync worker session."""
    candidate_ids = payload.get("candidate_ids")
    default_region_id = payload.get("default_region_id")
    default_channel_id = payload.get("default_channel_id")

    with SessionLocal() as db:
        try:
            out = build_dsi_resolution_plan_sync(
                db,
                job_id,
                candidate_ids=candidate_ids,
                default_region_id=default_region_id,
                default_channel_id=default_channel_id,
            )
            if on_progress is not None:
                total = int((out.get("summary") or {}).get("total") or len(out.get("rows") or []))
                on_progress(total, total)
            return out
        except Exception:
            logger.exception("dsi_resolution_plan_compute failed job_id=%s", job_id)
            raise
