"""Async dispatch for full customer merge (task_run ledger + dev poll cache)."""
...
from app.services.customer_full_merge import CustomerFullMergeError, confirm_customer_full_merge_sync
...
def dev_customer_full_merge_results() -> dict[str, dict[str, Any]]:
    return _dev_merge_task_results


def enqueue_customer_full_merge_confirm(payload: dict[str, Any]) -> tuple[str, bool]:
    from app.core.config import get_settings
    from app.core.dev_celery_logging import DEV_CELERY_LOGGER
    from app.services.task_run_ledger import (
        ENTITY_CUSTOMER_FULL_MERGE,
        TRANSPORT_BROKER,
    ...
    try:
        ...
        create_queued_task_run(
            task_run_id=task_id,
            task_name=TASK_NAME,
            entity_type=ENTITY_CUSTOMER_FULL_MERGE,
            entity_id=int(payload.get("survivor_id") or 0),
            transport=TRANSPORT_BROKER,
        )
        return task_id, True
    except Exception:
        logger.exception("customer full merge Celery enqueue failed")
        if settings.cip_dev_celery_dispatch == "in_process_thread":
            ...
            DEV_CELERY_LOGGER.warning(
                "ENQUEUE: customer full merge — in-process thread after broker failure (DEV ONLY)."
            )
            spawn_in_process_thread_with_ledger(
                task_run_id=task_id,
                thread_name="customer-full-merge",
