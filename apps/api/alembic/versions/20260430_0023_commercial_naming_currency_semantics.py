...
When 20260412_0001 ``create_all`` already materialized the renamed ORM, legacy columns
such as ``landed_cost_usd`` are absent; this migration skips rename DDL in that case.
"""
...
def upgrade() -> None:
    ...
    if has_table(insp, "commercial_sku_assumption"):
        if has_column(insp, "commercial_sku_assumption", "landed_cost_usd"):
            if not has_column(insp, "commercial_sku_assumption", "controlled_cost_currency_code"):
                op.add_column(
                    "commercial_sku_assumption",
                    sa.Column(
            ...
            op.execute(
                sa.text("ALTER TABLE commercial_sku_assumption RENAME COLUMN landed_cost_usd TO controlled_cost_amount")
            )
            op.execute(
                sa.text(
        ...
        # Else: ORM-shaped schema from 20260412_0001 (already renamed); nothing to do.

    insp = get_inspector(bind)

    # --- commercial_plan_line ---
    if has_table(insp, "commercial_plan_line"):
        if has_column(insp, "commercial_plan_line", "calc_sell_in_price_usd"):
            if not has_column(insp, "commercial_plan_line", "economics_calc_currency_code"):
                op.add_column(
                    "commercial_plan_line",
                    sa.Column(
                        "economics_calc_currency_code",
                        sa.String(length=8),
                        nullable=False,
                        server_default="USD",
                    ),
                )
            if not has_column(insp, "commercial_plan_line", "override_controlled_cost_currency_code"):
                op.add_column(
                    "commercial_plan_line",
            ...
            op.execute(
                sa.text(
                    "ALTER TABLE commercial_plan_line RENAME COLUMN override_landed_cost_usd TO override_controlled_cost_amount"
                )
            )
            op.execute(
                sa.text(
            ...
            op.execute(
                sa.text("ALTER TABLE commercial_plan_line RENAME COLUMN calc_sell_in_price_usd TO calc_oem_sell_in_amount")
            )
            op.execute(
                sa.text("ALTER TABLE commercial_plan_line RENAME COLUMN calc_buy_price_usd TO calc_distributor_net_amount")
            )
            op.execute(
                sa.text(
                    "ALTER TABLE commercial_plan_line RENAME COLUMN calc_promo_reserve_usd TO calc_campaign_support_reserve_amount"
                )
            )
            op.execute(
                sa.text(
                    "ALTER TABLE commercial_plan_line RENAME COLUMN calc_non_promo_reserve_usd TO calc_non_campaign_reserve_amount"
                )
            )
            op.execute(
                sa.text("ALTER TABLE commercial_plan_line RENAME COLUMN calc_internal_gp_usd TO calc_internal_gp_amount")
            )
            op.execute(
                sa.text(
                    "ALTER TABLE commercial_plan_line RENAME COLUMN calc_customer_gp_pct TO calc_customer_margin_input_pct"
                )
            )
            op.execute(
                sa.text(
...
def downgrade() -> None:
    ...
    if has_table(insp, "commercial_plan_line") and has_column(insp, "commercial_plan_line", "calc_oem_sell_in_amount"):
        op.execute(
            sa.text(
                "ALTER TABLE commercial_plan_line RENAME COLUMN calc_distributor_margin_input_pct TO calc_distributor_gp_pct"
            )
        )
        op.execute(
            sa.text(
                "ALTER TABLE commercial_plan_line RENAME COLUMN calc_customer_margin_input_pct TO calc_customer_gp_pct"
            )
        )
        ...
        op.execute(
            sa.text(
                "ALTER TABLE commercial_plan_line RENAME COLUMN override_controlled_cost_amount TO override_landed_cost_usd"
            )
        )
        if has_column(insp, "commercial_plan_line", "override_controlled_cost_currency_code"):
            op.drop_column("commercial_plan_line", "override_controlled_cost_currency_code")
        insp = get_inspector(bind)
        if has_column(insp, "commercial_plan_line", "economics_calc_currency_code"):
            op.drop_column("commercial_plan_line", "economics_calc_currency_code")

    insp = get_inspector(bind)
    if has_table(insp, "commercial_sku_assumption") and has_column(
        insp, "commercial_sku_assumption", "fx_plan_currency_per_cost_currency"
    ):
        op.execute(
            sa.text(
        ...
        if has_column(insp, "commercial_sku_assumption", "controlled_cost_currency_code"):
