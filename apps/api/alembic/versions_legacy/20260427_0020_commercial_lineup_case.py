...
def upgrade() -> None:
    ...
    if not has_table(insp, "commercial_lineup_case"):
        op.create_table(
    ...
    insp = get_inspector(bind)
    if not has_index(insp, "commercial_lineup_case", "ix_commercial_lineup_case_import_job_id"):
        op.create_index("ix_commercial_lineup_case_import_job_id", "commercial_lineup_case", ["import_job_id"])
    insp = get_inspector(bind)
    if not has_index(insp, "commercial_lineup_case", "ix_commercial_lineup_case_commercial_plan_id"):
        op.create_index("ix_commercial_lineup_case_commercial_plan_id", "commercial_lineup_case", ["commercial_plan_id"])

    insp = get_inspector(bind)
    if not has_table(insp, "commercial_lineup_line"):
        op.create_table(
    ...
    insp = get_inspector(bind)
    if not has_index(insp, "commercial_lineup_line", "ix_commercial_lineup_line_case_id"):
        op.create_index("ix_commercial_lineup_line_case_id", "commercial_lineup_line", ["case_id"])
    insp = get_inspector(bind)
    if not has_index(insp, "commercial_lineup_line", "ix_commercial_lineup_line_product_id"):
        op.create_index("ix_commercial_lineup_line_product_id", "commercial_lineup_line", ["product_id"])
    insp = get_inspector(bind)
    if not has_index(insp, "commercial_lineup_line", "ix_commercial_lineup_line_customer_id"):
        op.create_index("ix_commercial_lineup_line_customer_id", "commercial_lineup_line", ["customer_id"])
    insp = get_inspector(bind)
    if not has_index(insp, "commercial_lineup_line", "ix_commercial_lineup_line_distributor_id"):
...
def downgrade() -> None:
    ...
    for ix in (
        "ix_commercial_lineup_line_distributor_id",
        "ix_commercial_lineup_line_customer_id",
        "ix_commercial_lineup_line_product_id",
        "ix_commercial_lineup_line_case_id",
    ):
        insp = get_inspector(bind)
        if has_index(insp, "commercial_lineup_line", ix):
            op.drop_index(ix, table_name="commercial_lineup_line")
    insp = get_inspector(bind)
    if has_table(insp, "commercial_lineup_line"):
        op.drop_table("commercial_lineup_line")

    for ix in ("ix_commercial_lineup_case_commercial_plan_id", "ix_commercial_lineup_case_import_job_id"):
        insp = get_inspector(bind)
        if has_index(insp, "commercial_lineup_case", ix):
            op.drop_index(ix, table_name="commercial_lineup_case")
    insp = get_inspector(bind)
    if has_table(insp, "commercial_lineup_case"):
