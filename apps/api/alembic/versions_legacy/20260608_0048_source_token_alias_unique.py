...
def _assert_no_alias_conflicts(conn) -> None:
    ...
    if dup_dist:
        sample = ", ".join(f"{r[0]!r} (scope={r[1]}, rows={r[2]})" for r in dup_dist)
        raise RuntimeError(
            "Cannot add distributor alias uniqueness: duplicate approved alias rows in scope. "
            f"Dedupe first. Sample: {sample}"
        )
    ...
    if dup_cust:
        sample = ", ".join(
            f"{r[0]!r} (src={r[1]}, dist={r[2]}, rows={r[3]})" for r in dup_cust
        )
        raise RuntimeError(
            "Cannot add customer alias uniqueness: duplicate approved alias rows in scope. "
            f"Dedupe first. Sample: {sample}"
        )
