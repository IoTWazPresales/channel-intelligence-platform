...
Several ``fact_*`` columns and DSI tables may already exist from 20260412_0001 ``create_all``;
...
def upgrade() -> None:
    ...
    if not has_column(insp, "fact_sales_sellout", "unit_sellout_price_ex_tax_amount"):
        op.add_column(
            "fact_sales_sellout",
            sa.Column("unit_sellout_price_ex_tax_amount", sa.Numeric(18, 4), nullable=True),
        )
    insp = get_inspector(bind)
    if not has_column(insp, "fact_sales_sellout", "reported_revenue_amount"):
        op.add_column(
            "fact_sales_sellout",
            sa.Column("reported_revenue_amount", sa.Numeric(18, 4), nullable=True),
        )
    insp = get_inspector(bind)
    if not has_column(insp, "fact_sales_sellout", "computed_revenue_amount"):
        op.add_column(
            "fact_sales_sellout",
            sa.Column("computed_revenue_amount", sa.Numeric(18, 4), nullable=True),
        )
    insp = get_inspector(bind)
    if not has_column(insp, "fact_sales_sellout", "currency_code"):
        op.add_column("fact_sales_sellout", sa.Column("currency_code", sa.String(length=8), nullable=True))
    insp = get_inspector(bind)
    if not has_column(insp, "fact_sales_sellout", "source_import_job_id"):
        op.add_column("fact_sales_sellout", sa.Column("source_import_job_id", sa.Integer(), nullable=True))
    insp = get_inspector(bind)
    if not fk_exists(insp, "fact_sales_sellout", "fk_fact_sales_sellout_source_import_job_id_import_job"):
    ...
    insp = get_inspector(bind)
    if not has_column(insp, "fact_inventory_distributor", "source_import_job_id"):
        op.add_column(
            "fact_inventory_distributor",
            sa.Column("source_import_job_id", sa.Integer(), nullable=True),
        )
    insp = get_inspector(bind)
    if not fk_exists(insp, "fact_inventory_distributor", "fk_fact_inventory_distributor_source_import_job_id_import_job"):
        op.create_foreign_key(
    ...
    insp = get_inspector(bind)
    if not has_table(insp, "import_distributor_si_staging_line"):
        op.create_table(
    ...
    insp = get_inspector(bind)
    if not has_index(insp, "import_distributor_si_staging_line", "ix_import_distributor_si_staging_line_job_row"):
        op.create_index(
    ...
    insp = get_inspector(bind)
    if not has_index(insp, "import_distributor_si_staging_line", "ix_import_distributor_si_staging_line_job"):
        op.create_index(
            "ix_import_distributor_si_staging_line_job", "import_distributor_si_staging_line", ["import_job_id"]
        )

    insp = get_inspector(bind)
    if not has_table(insp, "import_entity_mapping_candidate"):
        op.create_table(
    ...
    insp = get_inspector(bind)
    if not has_index(insp, "import_entity_mapping_candidate", "ix_import_entity_mapping_candidate_job"):
        op.create_index("ix_import_entity_mapping_candidate_job", "import_entity_mapping_candidate", ["import_job_id"])

    insp = get_inspector(bind)
    if not has_table(insp, "customer_source_token_alias"):
        op.create_table(
    ...
    insp = get_inspector(bind)
    if not has_index(insp, "customer_source_token_alias", "ix_customer_source_token_alias_norm"):
...
def downgrade() -> None:
    ...
    if has_index(insp, "customer_source_token_alias", "ix_customer_source_token_alias_norm"):
        op.drop_index("ix_customer_source_token_alias_norm", table_name="customer_source_token_alias")
    insp = get_inspector(bind)
    if has_table(insp, "customer_source_token_alias"):
        op.drop_table("customer_source_token_alias")

    insp = get_inspector(bind)
    if has_index(insp, "import_entity_mapping_candidate", "ix_import_entity_mapping_candidate_job"):
        op.drop_index("ix_import_entity_mapping_candidate_job", table_name="import_entity_mapping_candidate")
    insp = get_inspector(bind)
    if has_table(insp, "import_entity_mapping_candidate"):
        op.drop_table("import_entity_mapping_candidate")

    insp = get_inspector(bind)
    if has_index(insp, "import_distributor_si_staging_line", "ix_import_distributor_si_staging_line_job"):
        op.drop_index("ix_import_distributor_si_staging_line_job", table_name="import_distributor_si_staging_line")
    insp = get_inspector(bind)
    if has_index(insp, "import_distributor_si_staging_line", "ix_import_distributor_si_staging_line_job_row"):
        op.drop_index("ix_import_distributor_si_staging_line_job_row", table_name="import_distributor_si_staging_line")
    insp = get_inspector(bind)
    if has_table(insp, "import_distributor_si_staging_line"):
        op.drop_table("import_distributor_si_staging_line")

    insp = get_inspector(bind)
    if fk_exists(insp, "fact_inventory_distributor", "fk_fact_inventory_distributor_source_import_job_id_import_job"):
        op.drop_constraint(
            "fk_fact_inventory_distributor_source_import_job_id_import_job",
            "fact_inventory_distributor",
            type_="foreignkey",
    ...
    insp = get_inspector(bind)
    if has_column(insp, "fact_inventory_distributor", "source_import_job_id"):
        op.drop_column("fact_inventory_distributor", "source_import_job_id")

    insp = get_inspector(bind)
    if fk_exists(insp, "fact_sales_sellout", "fk_fact_sales_sellout_source_import_job_id_import_job"):
        op.drop_constraint(
            "fk_fact_sales_sellout_source_import_job_id_import_job",
            "fact_sales_sellout",
            type_="foreignkey",
    ...
    for col in (
        "source_import_job_id",
        "currency_code",
        "computed_revenue_amount",
        "reported_revenue_amount",
        "unit_sellout_price_ex_tax_amount",
    ):
        insp = get_inspector(bind)
        if has_column(insp, "fact_sales_sellout", col):
            op.drop_column("fact_sales_sellout", col)
