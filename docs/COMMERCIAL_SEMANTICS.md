...
| A2-08 | **Paid vs owed recon** (BACKLOG-092) | IMPLEMENTED | **Owed** = Σ non-voided `cpor_case_line.ttl_support` in case currency (cancelled/rejected/superseded or no ttl → `owed_unknown`; never `result_qty × support_unit`). **Paid** = Σ linked `cpor_payment_evidence.amount` with mapped `payment_status` ∈ paid/processed/closed, same currency. **Outstanding** = owed − paid. Other-currency paid is FLAG `currency_mismatch`, never converted. Optional file column `owed_amount_file` shown vs CIP owed, never replaces it. `GET /cpor/cases/{id}/payment-recon` · Cases list columns · case tab **Payments / recon**. | CPOR |
| A2-X | Cost per **incremental** unit | **IMPLEMENTED 2026-08-17** | `support / max(0, result_qty − baseline_qty)`. Methods: `prior_window_same_sku_customer` (default), `velocity_extrapolate` (same scale), `comparable_median` (median week × case weeks). Weak baseline → **null + FLAG**. Distinct from A2-06. BACKLOG-089. | CPOR |

#### Non-computable register

| Former ID | Name | Reason | TRIGGER to reconsider |
|---|---|---|---|
| A2-03 | Claim rate (`owed ÷ approved`) | Settlement does **not** capture an **owed** amount independent of `support_unit × result_qty`. Claim evidence is units (+ optional `unit_price`); rollup writes `result_qty`; `ttl_result` is recomputed from the **same** `support_unit` as approved estimate. Owed÷approved money collapses to delivery rate (`result/estimate`). Building both would be two names for one number. **Distributor paid vs CIP owed** is A2-08 / BACKLOG-092 (payment evidence), not this claim-rate metric. | Settlement captures an **owed** amount **distinct from computed support** (and/or support-per-unit can differ between approval and settlement). |
...
| B1-04 | **Forecast method** | IMPLEMENTED (velocity + analogue + manual) | Taxonomy `{velocity, analogue, manual}` live; `blend` deferred. Precedence: override/manual > analogue > velocity. `/forecasts` primary CTA **Compute from history** (`POST /forecasts/compute-from-history`) runs velocity then analogue; paste/add are overrides. | same | Demand Forecast |
...
| B1-07 | **Forecast layer invariant** | IMPLEMENTED (15B) | Forecast is **never merged into actuals**. Separate table (`fact_demand_forecast`), separate labelled surface (`/forecasts`). Missing actual ≠ gap-filled with prediction. Compute-from-history writes forecast rows only; sell-out / CST / inventory facts are untouched. | — | Demand Forecast |
...
| ID | Metric / concept | Status | Formula / rule | Grain | Owner |
|---|---|---|---|---|---|
| B2-01 | **Net requirement** | IMPLEMENTED | `forecast_units` (dist×product rollup) − derived channel stock − in-transit (`open_order` + shipped∧`pod_date IS NULL`) + cover policy units. Cover weeks = `commercial_customer_term.target_cover_weeks` else tenant WOC default (4.0); dist×product cover = share-weighted weeks × weekly velocity. Query/body `target_cover_weeks` is an explicit override (`cover_override`). Pipeline never counts as stock. Optional A1 bias (UI toggle). `GET /api/v1/lineup/net-requirement`; apply → `POST /apply-net-requirement` → `fact_lineup_plan_item` (+ optional commercial case). Half-year period slots via `GET /half-year-periods`. | distributor × product × period | Line-up Planning (`/lineup`) |
...
| ID | Metric / concept | Status | Formula / rule | Grain | Owner |
|---|---|---|---|---|---|
| B4-01 | **Promo draft composition** | IMPLEMENTED | Compose A2 comps + B1 volume + intake-weighted MAC + B2 budget check into a **per-line** draft. `GET /cpor/intelligence/promo-plan-draft` + `POST …/recompute` emit `lines[]`. `POST …/create-case` with `lines[]` writes a **draft** CPOR case via existing case/line path (edited MAC → `cost_source=manual`, else `intake_weighted`). Dirty-flag is client-owned (Refresh never clobbers dirty). Cover edit is session `cover_override` only. Draft may warn on over-budget; hard enforce follows tenant profile (`confirm_over_budget`). Do not invent parallel economics. UI: `/promotions` B4 editable grid. | case / line | Promotions |
| B4-02 | **Intake-weighted MAC** (BACKLOG-094 / 15A) | IMPLEMENTED (composer; planner grid is 15C) | Suggested MAC = (A3 disti SOH × current CST `unit_mac`/`unit_cost` + PO-linked inbound `unit_price` landing in window) / (Aq+Bq). Empty B → `suggest_cost_basis` + `no_intake_evidence`. Display-only: sell-out Amount (`sellout_value_display_only`); buy-plan/lineup qty (`planned_supply_no_native_cost`); inbound `unit_price` WAVG as disti-WAC proxy (`dsi_wac_not_ingested`). Never DAP; never rewrite approved `cost_basis` (`detect_cost_basis_drift` FLAG). `GET /cpor/cases/{id}/lines/{line_id}/cost-suggest` → `intake_weighted`. Planner consume: 15C. | case line | CPOR |
