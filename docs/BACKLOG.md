...
| Field | Detail |
|-------|--------|
| **Status / parked** | **Done 2026-08-12** · Unit 11 — legacy `product_master_upsert` refuses; sole path is `product_master_workflow` |
| **Effort** | Medium–large |
...
| **What shipped** | `process_import_job_sync` raises for `template_slug=product_master`; `_process_product_master` raises BACKLOG-026; tests `test_product_master_pipeline_retired.py`. Import Centre continues to use `/imports/product-master` workflow. |
| **TRIGGER** | ~~A PM consolidation task is approved~~ — fired Unit 11 Q13=D. |
...
| Field | Detail |
|-------|--------|
| **Status / parked** | **Done** · 2026-09-04. Config-level `testTimeout: 15000` in `apps/web/vitest.config.ts`. Guard-on full suite **107 files / 583 passed**. |
| **Effort** | Small |
| **Source** | Prior: two `admin/imports/page.test.tsx` tests at 5s under full suite, ~2.1s in isolation. 2026-09-04: `DsiCandidateStewardPanel.test.tsx` resolve-product 5270ms timeout under full suite; isolation 1651ms. Same 5s Vitest default. |
| **Idea** | Raise the runner budget at config level. Per-test timeouts would paper over the same machine/suite-load defect in every new dialog test. |
| **Why it matters / deferrable** | Closed: 5s is too tight for this machine under 107-file jsdom load; isolation stays ~2s. |
| **What the work is** | Shipped `testTimeout: 15000`. Do not weaken assertions. |
| **Regression traps** | Do not disable the Cursor guard to “fix” vitest. Do not treat curl/API as UI smoke. |
| **Behavior to retain** | Existing AdminImportsPage and DsiCandidateStewardPanel coverage. |
| **Out of scope** | Splitting those pages. |
| **TRIGGER** | Closed. Reopen only if a test exceeds 15s under full-suite load. |
...
| Field | Detail |
|-------|--------|
| **Status / parked** | **CIP stdin/watchdog slice done** · 2026-09-04 (`6111634`). `hooks.json` restored; live `CONTROL_PLANE_PROTECTED` deny proven. EIF-repo items remain parked (BACKLOG-171). |
| **Effort** | Medium (EIF repo and/or CIP-owned hooks — separate grants) |
| **Source** | N-0013 `complete_n0013.py` synthetic `implementation_run`; `.eif/hook-guard.log`. Pre-fix harness 2026-09-04: hung 12.048s, empty stdout, no `HOOK_TIMEOUT`. Post-fix `6111634` / `prove_stdin.py`: hung-open `HOOK_TIMEOUT` 2.347s; complete-JSON held-open allow; closed-stdin allow / `ACTION_FORCE_VCS` / allow. Live restore deny: `CONTROL_PLANE_PROTECTED`. |
| **Idea** | Record, do **not** fix in `C:\AI\engineering-intelligence-framework`: (1) design nodes required a synthetic `implementation_run` to satisfy independence; (2) guard fail-closed + mute/timeout poisons an entire session; (3) tool-burst budget kills long autonomous runs. CIP-owned repair: cheap identity; crash vs policy labels; **stdin** (`6111634`) arms the watchdog before read, finishes on complete JSON, emits `HOOK_TIMEOUT` on hung/incomplete stdin (prove 2.347s / watchdog=2). Closed-stdin allow / `ACTION_FORCE_VCS` / allow unchanged. `PROGRAMME_GIT_STAGE` is not a reason code. CONSULT (opus CLI) **keep_true** on `beforeReadFile` `failClosed`. Identity on a closed pipe is ~0.57s — not a 10s class. |
| **Why it matters / deferrable** | CIP hook silent-kill path is closed. Remaining items are EIF-repo (synthetic `implementation_run`, CONSULT invocation records, tool-burst budget) — not CIP hook source. |
| **What the work is** | **CIP remaining:** none for stdin/watchdog. Do not set `beforeReadFile` `failClosed:false` (CONSULT keep_true). **EIF repo still parked:** BACKLOG-171. |
...
| Field | Detail |
|-------|--------|
| **Status / parked** | **Accepted** · 2026-09-04. Operator: N-0006 stays `proposed`. No synthetic `implementation_run`. FX enforcement is live in the product; the ledger gap is bookkeeping and waits for the EIF repair (BACKLOG-171). |
...
| **TRIGGER** | EIF repair session (BACKLOG-171). Do not lease N-0006 in CIP to stamp implement. |
...
## BACKLOG-171 — Programme cannot reconcile work that arrived out of order (EIF repair #1)

| Field | Detail |
|-------|--------|
| **Status / parked** | **Parked** · 2026-09-04; instance (4) recorded 2026-09-06. **First item for the EIF repair session.** Do **not** fix in CIP. Do not edit `C:\AI\engineering-intelligence-framework` from a CIP session. |
| **Effort** | Large (EIF event-model + independence semantics) |
| **Source** | Four CIP instances of one engine gap: (1) N-0013 synthetic `implementation_run` (`NS6_N0013_IMPL_BOUNDARY_20260903`) so independent gates could evaluate a design node whose implement stage was not the product change; (2) BACKLOG-169 — CONSULT caveat resolved in an audit file with no append-only event that can point at prior seq 287; (3) BACKLOG-170 — NS-1b `fx_mode` / `fx_settle_allowed` already shipped (Alembic `20260902_0020`) while N-0006 remains `proposed` with no lawful complete path without a post-hoc implement stamp; (4) 2026-09-06 container-migration wave — `program.py verify` ok rev 295, `frontier()` is only **N-0006**. `h_node_add` would accept a sibling leaf (no frontier-exclusivity check) and `frontier()` would then list both ids, which manufactures a parallel path around the only ready node (operator: do not start N-0006; do not manufacture a path). Completing any new `touches_existing` R≥2 node still requires first `node.stage → implement` (`stamp_implementation_boundary`) plus independent referent (`h_status` dest `complete` → `gates_ok`) — the same closed-handler gap. D-0008 under completed N-0013 is the architecture authority; there is no lawful covering implementation node to lease. N-0010/N-0011 remain `blocked` on BL-0001/BL-0002 `ref: N-0013` even though N-0013 is complete; D-0009 accepted N-0010 does not survive as a work container, but seq 295 ledger is uncommitted and `.eif/program/**` is CONTROL_PLANE_PROTECTED. Engine: closed `apply_event` handler set; `stamp_implementation_boundary` first `stage→implement` only; `UNKNOWN_EVENT` otherwise. `engine.py` `frontier` / `h_node_add` / `h_status` / `h_stage`. |
| **Idea** | The programme assumes work is recorded in stage order inside the ledger. It has no lawful event for “this already happened in git / in an audit file / in a later caveat.” CIP then either manufactures a boundary (N-0013) or parks the node (N-0006) or splits truth across PROGRAM_LOG vs audit (BACKLOG-169) or ships container migration without a leased node (instance 4). |
| **Why it matters / deferrable** | Every later node that ships ahead of the ledger, or that needs a post-hoc caveat, will repeat this. Deferrable until an authorised EIF-repo session. Operator accepted BACKLOG-170: N-0006 stays proposed; no synthetic stamp. |
| **What the work is** | In the EIF repo, design append-only event(s) that can (a) attach pre-existing implementation provenance without lying that this run wrote the code, (b) resolve a caveat against a prior seq without mutating `h_quality` evidence, (c) keep `UNKNOWN_EVENT` for undeclared types. Then host-upgrade CIP. Until then CIP records the gap in BACKLOG-169/170/171 only. |
...
## BACKLOG-181 — Port LabShell rail treatment into production CapabilityRail
...
| Field | Detail |
|-------|--------|
| **Status / parked** | **Closed — workspace retired** (2026-09-24 N-0044, operator decision D-c). What moved where: **settlement desk** (`SettlementDesk` / `SettlementDeskLive`) — FX anchor facts as an "Approved support" `DualMoney` headline figure (booked by/at or proposed rate/source) plus the FX basis line in the header meta; the settle-readiness chips at the top of the Next-action panel; `needs_reapproval` warning; End / Cancel (confirm dialog) from `allowed_next`, with "Open in planner" for pre-approval actions; intelligence-exclude switch; re-rollup from claims; include-out-of-window toggle on claim upload; claim diagnostics chips (out-of-window, unresolved products + tokens, CST divergence) and the import summary; unresolved count now reaches `SettlementConfirmDialog`; a lazy **Comparable cases** tab in `SettlementCaseTabs`. **PlanWorkspace** — approve-and-book-FX dialog (`fx_rate`, `confirm_over_budget_reapproval`), "Reapprove (over budget)" label and banner, FX mode toggle and proposed-rate edit (draft/rejected), PM `last_comment`; settle removed from the planner and routed to the desk ("Settle on the desk"). Not ported (display only, recorded): `workflow_status` and `export_version` header chips (export version is in the Exports tab), the first-6-flags chip row (PlanWorkspace has a flags panel), and per-line Ttl result local/USD money columns on the settlement grid. Deleted: `CporCaseWorkspace.tsx`, `CporFxAnchorPanel.tsx`; `SettlementPortfolioRead.tsx` (strict duplicate; rest of the orphan `SettlementContainer` subtree kept per D-0032); `CporPortfolioIntelligencePanel` moved and trimmed to `features/promotions-funding/PortfolioReadPanel.tsx`, mounted collapsed on the Case book (no incremental-unit tile, no support-bias block; reversible default, D-0032). No new endpoints. · *History:* **Partial — five tabs restored on the desk; workspace kept** (2026-09-21 N-0032: USD pivot (as a table), Events, Exports, Promo load recon and Payments/recon now mount under the settlement desk via `SettlementCaseTabs`; `CporCaseWorkspace` **stays** because the desk still lacks `CporFxAnchorPanel`, `CporSettleReadinessRow`, `CporComparableCasesPanel` and the lifecycle transition buttons — port those before deleting it) · **Decided** (2026-09-21 N-0030 audit: Warren — merge the five orphaned tabs onto the settlement desk; Stage 2.7; restores BACKLOG-093 promo-load recon) · **Open — product call, not a bug** · 2026-09-21 · grid/column parity audit §7.4 |
...
**2026-09-23 follow-up — shim built and tested; install deliberately held:**

**What shipped (EIF repo only, nothing installed into CIP yet):** `runtime/claude_code/eif_claude_adapter.py` (+ thin `.cmd`/`.sh` launchers) translates Claude Code's `PreToolUse`/`PostToolUse`/`PostToolUseFailure`/`Stop`/`SessionStart`/`SessionEnd`/`SubagentStart` hook JSON into the Cursor-shaped input `eif_guard.py` expects, invokes the target project's already-installed `.cursor/hooks/eif_guard.py` **unchanged** as a subprocess, and translates its allow/deny back into Claude Code's `hookSpecificOutput` schema. Fail-closed by construction: any `tool_name` or `hook_event_name` not explicitly reviewed and mapped is denied by the shim itself before the guard subprocess is ever invoked (`SHIM_TOOL_UNMAPPED` / `SHIM_EVENT_UNMAPPED`); every other failure (bad input, guard missing, guard crash, invalid guard output, guard timeout) is a bare `exit 2` + stderr reason with **no stdout**, matching Claude Code's documented "exit code 2 alone blocks" contract. The guard's own 8s self-denial watchdog (`WATCHDOG_DEFAULT_SEC` in `eif_guard.py`) is untouched; the shim adds its own 9.5s subprocess timeout as a backstop (headroom for process-spawn/I-O before the guard's watchdog thread would normally win). Recommended Claude Code hook `timeout` for `.claude/settings.json` when this is installed: **15s** (headroom above the shim's 9.5s for OS process-spawn latency). Test suite `tools/test_claude_code_adapter.py`, 13/13 passing, covering every mapped tool (`Bash→Shell`, `Read`, `Edit`/`MultiEdit`→`Write`, `NotebookEdit`→`Write` with `notebook_path`→`file_path` aliasing, `Glob`, `Grep`, `WebFetch`, `WebSearch`, `mcp__<server>__<tool>`→`MCP:<tool>`), the no-side-effect bypass list (`Task`, `AskUserQuestion`, `EnterPlanMode`, `ExitPlanMode`), every fail-closed path, the single-decision-emission guarantee, and a subagent call (`agent_id`/`agent_type` set) gated identically to a top-level call against `.eif/AUTONOMY_POLICY.md`. Committed `4a9ad80` on branch `feat/claude-code-hooks-shim`, pushed to `origin/feat/claude-code-hooks-shim` in the EIF repo (`C:\AI\engineering-intelligence-framework`) — **not merged, not installed anywhere.**

**Deferred, each with a trigger:**
- **Install into CIP** (`.claude/hooks/` + new `.claude/settings.json` registering `PreToolUse`/`SubagentStart` fail-closed via the shim, `PostToolUse`/`PostToolUseFailure`/`Stop`/`SessionStart`/`SessionEnd` mirroring the Cursor `hooks.json` table) — deliberately held this session per Warren's instruction. **Trigger:** Warren reviews the shim/tests in the EIF branch and says to proceed.
- ~~`AUTONOMY_POLICY.md` policy amendment~~ — **done 2026-09-23**, see follow-up below. Still outstanding: the install itself.
- **`browser_interact_origin_ok` / `is_browser_tool()` naming gap:** `eif_guard.py` recognizes a browser tool by `tool_name` starting with `browser_` (Cursor's own naming). Claude Code's `mcp__claude-in-chrome__*` tools (`navigate`, `computer`, `read_page`, …) don't carry that prefix after translation, so they get the base MCP-allowlist grant but **not** the extra loopback/local-fixture origin-scoping Cursor's own browser tools get. Low risk today (browser mode is `interact` but nothing in this policy currently exercises claude-in-chrome), real risk once claude-in-chrome is used for in-page mutation. **Trigger:** before granting claude-in-chrome interactive/mutating actions beyond observe/navigate.

**2026-09-23 follow-up — Warren-authorized control-plane change applied and compiled:**

Warren applied the `AUTONOMY_POLICY.md` edit himself (agent's Edit tool was refused by Claude Code's own auto-mode classifier, reason `Self-Modification`, on a file governing this session's own autonomy envelope — not routed around). Two changes, both Warren-authorized this session: (1) `tools/compile_policy.py`'s `CONTROL_PLANE` constant (EIF repo) gained `.claude/**` — committed `0742b29998d182a9f5a430a3270848711b53722e` on `feat/claude-code-hooks-shim`, pushed; this is the **actual** source of the compiled policy's `control_plane_paths` (see below), `AUTONOMY_POLICY.md` itself has no such key and none of its content feeds that field — confirmed by reading every `af.get(...)` call in the compiler. (2) `AUTONOMY_POLICY.md`'s `mcp.tools` gained `{tool_pattern: '*', server_pattern: 'mcp__claude-in-chrome'}` alongside the existing `browser_*` Cursor entry — verified before recompiling: diff contained only those two lines correctly nested, zero tab characters, front matter parses with `yaml.safe_load` and `mcp.tools` holds exactly those 2 entries. Recompiled (`python tools/compile_policy.py` from the EIF checkout against this project) — **policy_source_hash `c21be12c9babc6e6f13eb53409b14dc4f5be8908059223c24af3c3a5bc9dba84`**, cross-confirmed by an independent `Get-FileHash -Algorithm SHA256` (`C21BE12C...BA84`, same value). Trivial guard check post-recompile: an ordinary `Read` of `AGENTS.md` through the real installed `.cursor/hooks/eif_guard.cmd` returned `{"permission":"allow","reason_code":"TOOL_OK","decision_kind":"policy"}`, exit 0 — the compiled policy still admits normal work. `.eif/AUTONOMY_POLICY.md` + `.cursor/eif-runtime-policy.json` committed together, control-plane-only (no product/docs files) — `1f8211217691f3af8db10e048a1fe77acb81fd25` on `feat/ns-2-brief-nav-collapse`, pushed. **`.claude/` hooks still not installed** — this was explicitly held back per instruction.

**New deferred item — `CONTROL_PLANE_DEFAULTS` (`eif_guard.py`) and `CONTROL_PLANE` (`compile_policy.py`) have drifted apart, pre-existing and unrelated to today's `.claude/**` addition:** `eif_guard.py`'s own hardcoded list (used as a floor, unioned with the compiled policy's `control_plane_paths` at `eif_guard.py:868`) has three entries the compiler's list lacks: `.eif/runtime-session/**`, `.eif/node-scope.json`, `.eif/node-scope.lock`. Not a live gap for those three specifically (the guard's own list still protects them regardless of what's compiled), but it means the two lists can no longer be assumed identical, and a future edit to one that's meant to apply to both will silently only land in one. Today's `.claude/**` addition went to the compiler's list only (per Warren's instruction, and because `eif_guard.py` itself is off-limits this session) — that's fine functionally (still reaches the guard via the compiled `control_plane_paths` field) but it's now a fourth asymmetric entry, compiler-only rather than guard-only. **Trigger:** next time either list is edited for an unrelated reason — reconcile both at that point rather than letting a fifth entry drift in.
- **Adapter compiler / volume / `validate.py` extension / probe `--runtime` flag** — out of scope for this pass entirely, not started. **Trigger:** next time this shim needs to be re-derived from a policy change, or a second host besides CIP needs it.
