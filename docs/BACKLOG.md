...
- **`AUTONOMY_POLICY.md` policy amendment** (adding `.claude/**` to `control_plane_paths`, and an MCP allowlist entry for `mcp__claude-in-chrome__*`) — prepared and printed in this session's report, **not applied to the file**. It must land at/before the install, since without it every Claude Code MCP call denies by default (fail-closed, not a bug) and `.claude/**` itself has no extra protection beyond whatever ordinary `change_scopes` happens to allow. **Trigger:** same as install.
