...
| P1-D004 | shipment | pod_date propagation | continue | Active `shipment_evidence_line` often has `pod_date`, but `shipment_evidence_current` (observation `o.pod_date`) and `fact_inbound_shipment` leave POD null — Shipping landed/POD KPIs that read current/fact under-count landed. | Linked-PO sample ~62% POD on evidence vs ~0% via current/fact path. | open — fix view/fact COALESCE or apply-path write of `pod_date` (approved migration/apply); **do not** duplicate landing KPIs onto Plan vs Executed |
