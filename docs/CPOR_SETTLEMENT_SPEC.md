...
1. **`fact_inventory_customer` is empty; customer SOH is arriving and not landing there.**
   ...
   **2026-09-24 (N-0049):** the §5 SOH check now reads **`fact_customer_sellthrough` only** (`services/cpor/customer_soh_check.py`, returned as `soh_check` on `GET /cpor/cases/{id}/lines/{line_id}/cost-suggest`): latest CST week ≤ as-of with `reported_soh`, sites summed, SOH-weighted `unit_mac` else `unit_cost`, delta vs stored `cost_basis` reported (no tolerance flag); no CST row → `unavailable`. `fact_inventory_customer` is not a source for it. Still no derived customer×SKU MAC master (§5), so the check compares against line `cost_basis`.
