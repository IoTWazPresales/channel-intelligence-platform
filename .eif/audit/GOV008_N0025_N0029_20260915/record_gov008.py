"""Record independent GOV-008 for N-0025–N-0029.

Run/actor: GOV008_N0025_N0029_20260915 / gov-008
Windows: every event uses --payload-file.
Does not edit product source. Does not write cip.
"""
...
def event(node: str, name: str, payload: dict) -> subprocess.CompletedProcess[str]:
...
def record_node(
    ...
    *,
    ...
    if complete:
        event(node, "node.status", {"to": "complete"})
...
def main() -> None:
    record_node(
        ...
        summary=(
            ...
            "(no tenant user). Payments Back to Payments lens leftover. Dirty tree not this node."
        ),
        ...
        product="startWork.ts + StartWorkPanel + named leaves",
        signatures=[
            "overview_start_work_prime",
            "viewer_no_start_verbs",
            "cst_choose_file_after_provider",
            "steward_queue_lists_work",
            "payments_wizard_default",
            "terms_editor_default",
        ],
        interactions=[
            "click_import_a_lineup",
            "click_import_sell_through_to_choose_file",
            "click_import_shipping",
            "click_settle_to_case_book",
            "click_steward_queue",
            "login_viewer_no_verbs",
            "overview_390",
        ],
        states=["admin_start_work", "viewer_empty", "overview_390", "cst_choose_file"],
        comparison="start_work_reaches_existing_screens_steward_queue_lists_work",
        challenge=(
            "Challenged stuffing verbs into Attention. Independent live clicks opened existing "
            "screens. Steward queue is no longer an empty legacy leaf on the running tree. Path: " + EV
        ),
    )
    record_node(
        "N-0026",
