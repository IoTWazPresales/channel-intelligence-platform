...
- **Clones:** the `cip` DB role cannot `CREATE DATABASE` (permission denied, measured 2026-09-24). A "clone" means a scratch schema inside `cip_test` (schema-only + data-only `pg_dump` of just the needed tables from cip, rewritten into the scratch schema), dropped afterwards; `cip_test.public` stays untouched. N-0039's `IMPL.md` shows the pattern. Stale full clones also exist and may be written to when your prompt assigns one (never `cip`): e.g. `cip_ns4_settle_clone` (alembic 0020, data to 2026-09-02), `cip_merged_leftover_repair` (0019, 2026-08-19); state the clone's vintage next to every number and also print today's read-only cip count.
