...
- Automation tabs are often `document.visibilityState = hidden`, and the app's queries do not start in a hidden tab (pages sit on "Loading…" with zero `/api/` requests). Take a `screenshot` of your tab first to bring it to the foreground, then wait; this is an environment artifact, not a product defect (measured 2026-09-24 on cases 46 and 311).
